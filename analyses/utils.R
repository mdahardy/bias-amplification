    ...
    '../data/experiment_1.csv' %>%
      ...
      transmute(
        ...
        condition = case_when(
          ...
          condition == 'ASO:W-U' ~ 'asocial_motivated',
          condition == 'SOC:N-U' ~ 'social_control',
          TRUE ~ 'social_motivated'
        )
      ) %>% 
  ...
  return(
    '../data/experiment_2.csv' %>%
      ...
      transmute(
        ...
        social_info_green = k_chose_green/8,
        condition = case_when(
          grepl('ASO',condition,fixed=T) ~ 'asocial_motivated',
...
z_stat_string = function(estimate,z_statistic,p_value){
  ...
  return(paste(estimate_string,z_string,p_string,sep=', '))
}

fit_tie_model_and_write = function(all_data,formula_string,stats_dir){
  model_coefs = formula_string %>%
    as.formula() %>%
...
write_z_statistic = function(beta_estimate,z_statistic,p_val,stats_dir,output_name){
  z_string = z_stat_string(beta_estimate,z_statistic,p_val)
